...
if(WITH_GSTREAMER AND NOT WITH_GSTREAMER_0_10)
  if(WIN32)
    ...
    if(HAVE_GSTREAMER_BASE)
      ocv_append_build_options(VIDEOIO GSTREAMER_BASE)
    endif()
    ocv_check_modules(GSTREAMER_VIDEO gstreamer-video-1.0)
    if(HAVE_GSTREAMER_VIDEO)
      ocv_append_build_options(VIDEOIO GSTREAMER_VIDEO)
    endif()
    ocv_check_modules(GSTREAMER_APP gstreamer-app-1.0)
    if(HAVE_GSTREAMER_APP)
      ocv_append_build_options(VIDEOIO GSTREAMER_APP)
    endif()
    ocv_check_modules(GSTREAMER_RIFF gstreamer-riff-1.0)
    if(HAVE_GSTREAMER_RIFF)
      ocv_append_build_options(VIDEOIO GSTREAMER_RIFF)
    endif()
    ocv_check_modules(GSTREAMER_PBUTILS gstreamer-pbutils-1.0)
    if(HAVE_GSTREAMER_PBUTILS)
      ocv_append_build_options(VIDEOIO GSTREAMER_PBUTILS)
    endif()
...
if(WITH_GSTREAMER AND NOT HAVE_GSTREAMER OR WITH_GSTREAMER_0_10)
    ocv_check_modules(GSTREAMER_BASE gstreamer-base-0.10)
    if(HAVE_GSTREAMER_BASE)
      ocv_append_build_options(VIDEOIO GSTREAMER_BASE)
    endif()
    ocv_check_modules(GSTREAMER_VIDEO gstreamer-video-0.10)
    if(HAVE_GSTREAMER_VIDEO)
      ocv_append_build_options(VIDEOIO GSTREAMER_VIDEO)
    endif()
    ocv_check_modules(GSTREAMER_APP gstreamer-app-0.10)
    if(HAVE_GSTREAMER_APP)
      ocv_append_build_options(VIDEOIO GSTREAMER_APP)
    endif()
    ocv_check_modules(GSTREAMER_RIFF gstreamer-riff-0.10)
    if(HAVE_GSTREAMER_RIFF)
      ocv_append_build_options(VIDEOIO GSTREAMER_RIFF)
    endif()
    ocv_check_modules(GSTREAMER_PBUTILS gstreamer-pbutils-0.10)
    if(HAVE_GSTREAMER_PBUTILS)
      ocv_append_build_options(VIDEOIO GSTREAMER_PBUTILS)
    endif()
